...
export class WorkspacesManager {
  ...
  public async getWorkspace(name: string): Promise<GetWorkspaceResponse> {
    ...
    this.logger.info({ msg: 'getting workspace', name, logContext: logCtx });
  ...
  public async deleteWorkspace(name: string, isRecursive: boolean): Promise<void> {
    ...
    this.logger.info({ msg: 'deleteing workspace', name, logContext: logCtx });
  ...
  public async createWorkspace(name: string): Promise<void> {
    ...
    this.logger.info({ msg: 'creating workspace', name, logContext: logCtx });
  ...
  public async updateWorkspace(oldName: string, newName: string): Promise<void> {
    ...
    this.logger.info({ msg: 'updating workspace', oldName, newName, logContext: logCtx });
    ...
    if (!doesNewNameExists) {
      ...
      throw new ConflictError(`Cant change workspace ${oldName} to ${newName} , there is already a workspace named ${newName}`);
    }
  }
